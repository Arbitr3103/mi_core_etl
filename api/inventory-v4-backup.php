<?php
/**
 * API endpoint для Ozon v4 синхронизации остатков - ИСПРАВЛЕННАЯ ВЕРСИЯ
 */
...
class InventoryV4API {
    ...
    private function getMarketingAnalytics() {
        try {
            // 1. Топ товаров по остаткам (хиты продаж) - ПРАВИЛЬНАЯ ЛОГИКА
            ...
            // 2. Товары требующие внимания - ПРАВИЛЬНАЯ ЛОГИКА
    ...
    private function getProductDetails() {
        try {
            ...
            $whereClause = "WHERE i.source IN ('Ozon', 'Ozon_Analytics')";
            $params = [];
            
            if (!empty($search)) {
                $whereClause .= " AND (i.sku LIKE ? OR i.product_id LIKE ?)";
                $searchParam = "%{$search}%";
                $params = [$searchParam, $searchParam];
            }
            
            // ПРАВИЛЬНАЯ ЛОГИКА для детального просмотра
            $sql = "
                SELECT 
                    i.product_id,
                    i.sku,
                ...
                LEFT JOIN product_names p ON i.product_id = p.product_id AND i.sku = p.sku
                {$whereClause}
                ORDER BY i.sku, i.warehouse_name
                LIMIT {$offset}, {$limit}
            ";
            ...
            foreach ($products as $product) {
                $key = $product['display_id'] . '_' . $product['sku'];
