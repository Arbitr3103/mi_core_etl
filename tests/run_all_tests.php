...
require_once __DIR__ . '/MarketplaceDetectorTest.php';
...
class TestRunner {
    ...
    public function runAllTests() {
        ...
        echo "Тестируемые классы: Region, CarFilter, CountryFilterAPI, MarketplaceDetector\n";
        echo "=" . str_repeat("=", 80) . "\n\n";
        ...
        // Запуск тестов MarketplaceDetector
        echo "🔧 ТЕСТИРОВАНИЕ КЛАССА MarketplaceDetector\n";
        echo "=" . str_repeat("=", 80) . "\n";
        
        try {
            $marketplaceTest = new MarketplaceDetectorTest();
            
            // Перехватываем вывод для подсчета результатов
            ob_start();
            $marketplaceTest->runAllTests();
            $marketplaceOutput = ob_get_clean();
            
            echo $marketplaceOutput;
            
            // Подсчитываем результаты MarketplaceDetector тестов
            $this->countTestResults($marketplaceOutput);
            
        } catch (Exception $e) {
            echo "❌ КРИТИЧЕСКАЯ ОШИБКА В ТЕСТАХ MarketplaceDetector: " . $e->getMessage() . "\n\n";
            $this->failedTests++;
        }
    ...
    private function printFinalReport() {
        ...
        echo "  ✅ Requirement 6.2: Обработка отсутствующих данных маркетплейса\n";
        echo "  ✅ Requirement 6.4: Валидация параметров маркетплейса\n";
        
        // Определяем общий статус
        if ($this->failedTests === 0) {
            echo "\n🎉 ВСЕ ТЕСТЫ ПРОЙДЕНЫ УСПЕШНО!\n";
            echo "Классы Region, CarFilter, CountryFilterAPI и MarketplaceDetector готовы к использованию.\n";
            echo "Система фильтрации по стране изготовления и разделения данных по маркетплейсам полностью протестирована.\n";
        } else {
