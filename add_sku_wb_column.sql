...
ALTER TABLE dim_products ADD COLUMN sku_wb VARCHAR(50) NULL AFTER sku_ozon;
ALTER TABLE dim_products ADD COLUMN name VARCHAR(500) NULL AFTER product_name;
ALTER TABLE dim_products ADD COLUMN brand VARCHAR(255) NULL AFTER name;
ALTER TABLE dim_products ADD COLUMN category VARCHAR(255) NULL AFTER brand;

-- Добавляем индексы для новых колонок
ALTER TABLE dim_products ADD INDEX idx_sku_wb (sku_wb);
ALTER TABLE dim_products ADD INDEX idx_name (name(100));
ALTER TABLE dim_products ADD INDEX idx_brand (brand);
ALTER TABLE dim_products ADD INDEX idx_category (category);
