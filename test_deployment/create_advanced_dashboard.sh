...
echo "🎨 СОЗДАНИЕ ПРОДВИНУТОГО ДАШБОРДА КАК В OZON"
...
try {
    ...
    if ($search) {
        $where_conditions[] = "(p.name LIKE ? OR p.sku_ozon LIKE ? OR i.product_id LIKE ?)";
        $params[] = "%$search%";
        $params[] = "%$search%";
        $params[] = "%$search%";
    }
            ...
            <form method="GET" class="filters">
                ...
                <div class="filter-group">
                    ...
                    <input type="text" name="search" value="<?= htmlspecialchars($search) ?>" 
                           placeholder="Название, SKU, ID..." class="filter-input" style="width: 200px;">
                </div>
                    ...
                    <?php foreach($data as $row): ?>
                        <tr>
                            <?php if($active_tab === 'warehouse_products'): ?>
                                ...
                                <td><?= htmlspecialchars($row['sku_ozon'] ?? '-') ?></td>
...
echo "✅ Продвинутый дашборд создан!"
