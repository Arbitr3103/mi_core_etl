# Руководство по импорту себестоимости товаров
...
### UPSERT логика (умное обновление справочника):
Система работает по принципу "найти по штрихкоду → обновить или создать":
1. **Основной ключ** - штрихкод (`barcode`) как универсальный идентификатор
2. **Если товар найден** - обновляет `cost_price` и `sku_internal`
3. **Если товар не найден** - создает новую запись в справочнике
4. **Совместимость** - работает для товаров из Ozon, WB и других источников
...
## Процесс обработки (UPSERT логика)

1. **Поиск файла** - ищется `cost_price.xlsx` в `uploads/`
2. **Валидация** - проверка структуры и данных Excel
3. **UPSERT в БД** - умное обновление справочника:
   - Поиск товара по штрихкоду: `SELECT id FROM dim_products WHERE barcode = ?`
   - Если найден: `UPDATE dim_products SET cost_price = ?, sku_internal = ? WHERE id = ?`
   - Если не найден: `INSERT INTO dim_products (barcode, sku_internal, cost_price) VALUES (?, ?, ?)`
4. **Архивация** - перемещение в `uploads/archive/cost_price_YYYYMMDD_HHMMSS.xlsx`
5. **Отчет** - статистика обновленных/созданных/ошибок товаров
